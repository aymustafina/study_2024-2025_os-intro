...
Устанавливаю необходимое ПО с помощью команд dnf install pass pass-otp (рис. [-@fig:001]).

![Установка ПО](image/1.png){#fig:001 width=70%}

И еще dnf install gopass (рис. [-@fig:002]).

![Установка ПО](image/2.png){#fig:002 width=70%}  

## Настройка

Настройка ключей GPG, просматриваю список ключей gpg --list-secret-keys (рис. [-@fig:003]).
...
Синхронизация с git с помощью pass git init (рис. [-@fig:005]).
...
Также задаю адрес репозитория на хостинге pass git remote add origin git@github.com:<git_username>/<git_repo>.git (рис. [-@fig:007]).
...
С помощью pass git status проверяю статус синхронизации (рис. [-@fig:008]).

![Синхронизация](image/9.png){#fig:008 width=70%}

Вручную делаю коммит: cd ~/.password-store/  git add    git commit -am 'edit manually'      git push (рис. [-@fig:010]).

![Коммит](image/10.png){#fig:010 width=70%}

## Настройка интерфейса с браузером

Устанавливаю программу, обеспечивающую интерфейс native messaging (рис. [-@fig:011]).

![Программа](image/11.png){#fig:011 width=70%}

И еще другая программа (рис. [-@fig:012]).
...
sudo dnf -y install \ dunst \  fontawesome-fonts \     powerline-fonts \  light \   fuzzel \  swaylock \    kitty \  waybar swaybg \    wl-clipboard \     mpv \    grim \   slurp (рис. [-@fig:014]).
...
sudo dnf search iosevka (рис. [-@fig:015]).

![Шрифты](image/15.png){#fig:015 width=70%}

sudo dnf install iosevka-fonts iosevka-aile-fonts iosevka-curly-fonts iosevka-slab-fonts (рис. [-@fig:016]).

![Шрифты](image/16.png){#fig:016 width=70%}

## Установка и создание собственного репозитория с помощью утилит

Установливаю бинарный файл. Скрипт определяет архитектуру процессора и операционную систему и скачивает необходимый файл: sh -c "$(wget -qO- chezmoi.io/get)" 
Создаю свой репозиторий для конфигурационных файлов на основе шаблона: 
gh repo create dotfiles --template="yamadharma/dotfiles-template" --private (рис. [-@fig:017]).

![Бинарный файл и создание репозитория](image/17.png){#fig:017 width=70%}

## Подключение репозитория к своей системе

Инициализирую chezmoi с репозиторием dotfiles  chezmoi init git@github.com:<username>/dotfiles.git (рис. [-@fig:018]).

![dotfiles](image/18.png){#fig:018 width=70%}

Изменения меня устраивают, поэтому ввожу необходимую команду (рис. [-@fig:019]).
...
Для пунктов использование chezmoi на нескольких машинах, настройка новой машины с помощью одной команды, еедневные операции c chezmoi. (рис. [-@fig:020])
